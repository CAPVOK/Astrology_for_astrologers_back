...
func (r *Repository) DeletePlanet(planetID, userID uint) error {
	...
	if err := r.db.Table("planets").Where("planet_id = ? AND planet_status = ?", planetID, model.PLANET_STATUS_ACTIVE).First(planet).Error; err != nil {
		return errors.New("планета не найден или уже удален")
	}

	planet.PlanetStatus = model.PLANET_STATUS_DELETED

	if err := r.db.Table("planets").Save(planet).Error; err != nil {
